...
# and [Bobak et al.](https://arxiv.org/abs/2001.11897)
# that a $d$ dimensional unitary can be approximated 
# with parameters $N$ of order $O(d^2)$.
...
#  \end{equation}$
#   for every parameter $t_{k}$, where $\alpha$ is the learning rate. 
...
# The papers claim that if the dimension, N, of the parameters is of order $O(d^{2})$, where $d$ is the dimension of the unitary, then $U(\vec{t},\vec{\tau})$ is constructed _efficiently_. We verify this claim with $d = 2$ and $N = 4$. 
